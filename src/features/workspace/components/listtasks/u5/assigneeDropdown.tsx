...
export function AssigneeDropdown({
  ...
  const selected = members.filter((member) => current.includes(member.id))
  ...
  function toggle(memberId: string) {
    if (current.includes(memberId)) {
      commit(current.filter((id) => id !== memberId))
    } else {
      commit([...current, memberId])
    }
  }
    ...
    >
      ...
      <section aria-label="Assignees" className="py-2">
        ...
        <ul aria-label="Assignees list" className="space-y-0.5" role="listbox">
          {filtered.map((member) => {
            const active = current.includes(member.id)
            return (
              <li
                key={member.id}
